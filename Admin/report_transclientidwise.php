...
$selectedProject = $_POST['client_name'] ?? '';
...
<div class="card shadow">
    <div class="card-header text-center">
        <h5><b>Report Transaction Details (Client Wise)</b></h5>
    </div>
...
<div class="row">
    ...
    <div class="col-md-4">
        ...
        <select name="client_name" id="client_name" class="form-control" onchange="submitForm()" required>
            <option value="">-- Select Clients --</option>
            <?php
                $q = "SELECT DISTINCT client_name FROM clients";
                $r = mysqli_query($conn, $q);
                while($row = mysqli_fetch_assoc($r)){
                    $p = $row['client_name'];
                    $sel = ($p == $selectedProject) ? "selected" : "";
...
if(!empty($selectedProject) && !empty($selectedYear)){
    ...
    WHERE m.client_name='$selectedProject'
...
let pending = 1; // company logo always

if (CLIENT_LOGO) pending++; // increment if client logo exists
